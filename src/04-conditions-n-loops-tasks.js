...
function findFirstSingleChar(str) {
  ...
  const charCount = chars.reduce((acc, ch) => {
    acc[ch] = (acc[ch] || 0) + 1;
    return acc;
  }, {});

  const firstUnique = chars.find((ch) => charCount[ch] === 1);
...
function getCommonDirectoryPath(pathes) {
  const minLength = Math.min(...pathes.map((p) => p.length));
  let common = '';

  for (let i = 0; i < minLength; i += 1) {
    const char = pathes[0][i];
    if (pathes.every((p) => p[i] === char)) {
      common += char;
    } else {
      break;
    }
...
function getMatrixProduct(m1, m2) {
  return m1.map((row, i) =>
    m2[0].map((_item, j) =>
      row.reduce((acc, _el, n) => acc + m1[i][n] * m2[n][j], 0)
    )
  );
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    ...
    for (let j = 0; j < position[i].length; j += 1) {
      if (j === 0) {
        col0.push(position[i][j]);
      } else if (j === 1) {
        col1.push(position[i][j]);
      } else if (j === 2) {
        col2.push(position[i][j]);
      }

      if (i === j) {
        d1.push(position[i][j]);
      }
